// ============================================================
// GMAIL — Authentification Google OAuth + lecture des emails
// ============================================================
...
const Gmail = {
  ...
  async authenticate(existingAccountId = null) {
    return new Promise((resolve, reject) => {
      ...
      const interval = setInterval(() => {
        try {
          ...
          // Cross-origin pendant la redirection, on attend
        }
      }, 300);
  ...
  saveToken(accountId, tokenData) {
    localStorage.setItem(this.TOKEN_KEY_PREFIX + accountId, JSON.stringify({
      ...tokenData,
      savedAt: Date.now(),
    }));
  },
  ...
  getToken(accountId) {
    try {
      ...
      if (Date.now() > data.expiresAt - 60000) return null; // expire dans < 1 min
      return data.accessToken;
    } catch { return null; }
  },

  removeToken(accountId) {
    localStorage.removeItem(this.TOKEN_KEY_PREFIX + accountId);
  },
  ...
  async fetchVintedEmails(accessToken, sinceDate = null) {
    ...
    // Récupération des détails en batch (5 à la fois)
    const emails = [];
